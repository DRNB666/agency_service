...
@RestController
@RequestMapping(value = "user/userVipFlow", produces = "text/plain;charset=utf-8")
public class UserVipFlowController {
    ...
    public String list(PageInfo pageInfo) {
        PageResult<UserVipFlow> pageResult = new PageUtil<UserVipFlow>(pageInfo).startPage((page, queryWrapper) -> userVipFlowService.page(page, queryWrapper));
    ...
    public String addOrUpdate(UserVipFlow userVipFlow) {
        userVipFlow.setCreateTime(null).setStatus(null).setUpdateTime(null);
        if (request.getRequestURI().contains("update")) {
            if (!CommUtil.checkNull(userVipFlow.getId())) {
                return ReturnBody.error();
            }
        }
        if (!userVipFlowService.saveOrUpdate(userVipFlow)) {
            return ReturnBody.error();
        }
    ...
    public String delete(String id) {
        return userVipFlowService.removeById(id) ? ReturnBody.success() : ReturnBody.error();
    }
