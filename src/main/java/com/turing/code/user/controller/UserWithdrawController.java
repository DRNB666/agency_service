...
@RestController
@RequestMapping(value = "user/userWithdraw", produces = "text/plain;charset=utf-8")
public class UserWithdrawController {
    ...
    public String list(PageInfo pageInfo) {
        PageResult<UserWithdraw> pageResult = new PageUtil<UserWithdraw>(pageInfo).startPage((page, queryWrapper) -> userWithdrawService.page(page, queryWrapper));
    ...
    public String addOrUpdate(UserWithdraw userWithdraw) {
        userWithdraw.setCreateTime(null).setStatus(null);
        if (request.getRequestURI().contains("update")) {
            if (!CommUtil.checkNull(userWithdraw.getId())) {
                return ReturnBody.error();
            }
        }
        if (!userWithdrawService.saveOrUpdate(userWithdraw)) {
            return ReturnBody.error();
        }
    ...
    public String delete(String id) {
        return userWithdrawService.removeById(id) ? ReturnBody.success() : ReturnBody.error();
    }
