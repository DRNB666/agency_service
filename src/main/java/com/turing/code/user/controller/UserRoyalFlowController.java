...
@RestController
@RequestMapping(value = "user/userRoyalFlow", produces = "text/plain;charset=utf-8")
public class UserRoyalFlowController {
    ...
    public String list(PageInfo pageInfo) {
        PageResult<UserRoyalFlow> pageResult = new PageUtil<UserRoyalFlow>(pageInfo).startPage((page, queryWrapper) -> userRoyalFlowService.page(page, queryWrapper));
    ...
    public String addOrUpdate(UserRoyalFlow userRoyalFlow) {
//        userRoyalFlow.setCreateTime(null).setStatus(null).setVersion(null).setUpdateTime(null);
        if (request.getRequestURI().contains("update")) {
            if (!CommUtil.checkNull(userRoyalFlow.getId())) {
                return ReturnBody.error();
            }
        }
        if (!userRoyalFlowService.saveOrUpdate(userRoyalFlow)) {
            return ReturnBody.error();
        }
    ...
    public String delete(String id) {
        return userRoyalFlowService.removeById(id) ? ReturnBody.success() : ReturnBody.error();
    }
