...
import com.turing.code.system.service.SysBankService;
...
@RestController
@RequestMapping(value = "system/sysBank", produces = "text/plain;charset=utf-8")
public class SysBankController {
    ...
    public String list(PageInfo pageInfo) {
        PageResult<SysBank> pageResult = new PageUtil<SysBank>(pageInfo).startPage((page, queryWrapper) -> sysBankService.page(page, queryWrapper));
    ...
    public String addOrUpdate(SysBank sysBank) {
        sysBank.setCreateTime(null).setStatus(null);
        if (request.getRequestURI().contains("update")) {
            if (!CommUtil.checkNull(sysBank.getId())) {
                return ReturnBody.error();
            }
        }
        if (!sysBankService.saveOrUpdate(sysBank)) {
            return ReturnBody.error();
        }
    ...
    public String delete(String id) {
        return sysBankService.removeById(id) ? ReturnBody.success() : ReturnBody.error();
    }
