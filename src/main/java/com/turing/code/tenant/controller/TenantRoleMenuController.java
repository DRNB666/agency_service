...
@RestController
@RequestMapping(value = "tenant/tenantRoleMenu", produces = "text/plain;charset=utf-8")
public class TenantRoleMenuController {
    ...
    public String list(PageInfo pageInfo) {
        PageResult<TenantRoleMenu> pageResult = new PageUtil<TenantRoleMenu>(pageInfo).startPage((page, queryWrapper) -> tenantRoleMenuService.page(page, queryWrapper));
    ...
    public String addOrUpdate(TenantRoleMenu tenantRoleMenu) {
        tenantRoleMenu.setCreateTime(null).setStatus(null).setVersion(null).setUpdateTime(null);
        if (request.getRequestURI().contains("update")) {
            if (!CommUtil.checkNull(tenantRoleMenu.getId())) {
                return ReturnBody.error();
            }
        }
        if (!tenantRoleMenuService.saveOrUpdate(tenantRoleMenu)) {
            return ReturnBody.error();
        }
    ...
    public String delete(String id) {
        return tenantRoleMenuService.removeById(id) ? ReturnBody.success() : ReturnBody.error();
    }
