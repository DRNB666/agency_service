...
import com.turing.code.tenant.service.TenantShopCategoryService;
...
@RestController
@RequestMapping(value = "tenant/tenantShopCategory", produces = "text/plain;charset=utf-8")
public class TenantShopCategoryController {
    ...
    public String list(PageInfo pageInfo) {
        PageResult<TenantShopCategory> pageResult = new PageUtil<TenantShopCategory>(pageInfo).startPage((page, queryWrapper) -> {
            tenantShopCategoryService.page(page, queryWrapper);
        });
    ...
    public String addOrUpdate(TenantShopCategory tenantShopCategory) {
        tenantShopCategory.setCreateTime(null).setStatus(null).setUpdateTime(null);
        if (request.getRequestURI().contains("update")) {
            if (!CommUtil.checkNull(tenantShopCategory.getId())) {
                return ReturnBody.error();
            }
        }
        if (!tenantShopCategoryService.saveOrUpdate(tenantShopCategory)) {
            return ReturnBody.error();
        }
    ...
    public String delete(String id) {
        return tenantShopCategoryService.removeById(id) ? ReturnBody.success() : ReturnBody.error();
    }
