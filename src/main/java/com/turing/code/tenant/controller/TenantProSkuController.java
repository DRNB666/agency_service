...
@RestController
@RequestMapping(value = "tenant/tenantProSku", produces = "text/plain;charset=utf-8")
public class TenantProSkuController {
    ...
    public String list(PageInfo pageInfo) {
        PageResult<TenantProSku> pageResult = new PageUtil<TenantProSku>(pageInfo).startPage((page, queryWrapper) -> tenantProSkuService.page(page, queryWrapper));
    ...
    public String addOrUpdate(TenantProSku tenantProSku) {
        tenantProSku.setCreateTime(null).setStatus(null).setUpdateTime(null);
        if (request.getRequestURI().contains("update")) {
            if (!CommUtil.checkNull(tenantProSku.getId())) {
                return ReturnBody.error();
            }
        }
        if (!tenantProSkuService.saveOrUpdate(tenantProSku)) {
            return ReturnBody.error();
        }
    ...
    public String delete(String id) {
        return tenantProSkuService.removeById(id) ? ReturnBody.success() : ReturnBody.error();
    }
