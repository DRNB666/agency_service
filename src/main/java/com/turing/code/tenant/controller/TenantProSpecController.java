...
@RestController
@RequestMapping(value = "tenant/tenantProSpec", produces = "text/plain;charset=utf-8")
public class TenantProSpecController {
    ...
    public String list(PageInfo pageInfo) {
        PageResult<TenantProSpec> pageResult = new PageUtil<TenantProSpec>(pageInfo).startPage((page, queryWrapper) -> tenantProSpecService.page(page, queryWrapper));
    ...
    public String addOrUpdate(TenantProSpec tenantProSpec) {
        tenantProSpec.setCreateTime(null).setStatus(null);
        if (request.getRequestURI().contains("update")) {
            if (!CommUtil.checkNull(tenantProSpec.getId())) {
                return ReturnBody.error();
            }
        }
        if (!tenantProSpecService.saveOrUpdate(tenantProSpec)) {
            return ReturnBody.error();
        }
    ...
    public String delete(String id) {
        return tenantProSpecService.removeById(id) ? ReturnBody.success() : ReturnBody.error();
    }
