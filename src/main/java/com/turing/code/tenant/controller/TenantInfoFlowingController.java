...
@RestController
@RequestMapping(value = "tenant/tenantInfoFlowing", produces = "text/plain;charset=utf-8")
public class TenantInfoFlowingController {
    ...
    public String list(PageInfo pageInfo) {
        PageResult<TenantInfoFlowing> pageResult = new PageUtil<TenantInfoFlowing>(pageInfo).startPage((page, queryWrapper) -> tenantInfoFlowingService.page(page, queryWrapper));
    ...
    public String addOrUpdate(TenantInfoFlowing tenantInfoFlowing) {
        tenantInfoFlowing.setCreateTime(null).setStatus(null);
        if (request.getRequestURI().contains("update")) {
            if (!CommUtil.checkNull(tenantInfoFlowing.getId())) {
                return ReturnBody.error();
            }
        }
        if (!tenantInfoFlowingService.saveOrUpdate(tenantInfoFlowing)) {
            return ReturnBody.error();
        }
    ...
    public String delete(String id) {
        return tenantInfoFlowingService.removeById(id) ? ReturnBody.success() : ReturnBody.error();
    }
